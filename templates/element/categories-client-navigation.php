...
<hr>
<?php
    if($service_id == 1 && isset($activeUser['client_id']) && $activeUser['client_id'] == 6 && in_array($activeUser['role_id'], CLIENT_ALL)){
        $percentage = '';
        $myClients = $this->User->getClients($activeUser['contractor_id']);
        $percentage = $this->User->getHuntonPercentage($activeUser['contractor_id']);
        if($service_id == 1 && in_array(6, $myClients)){
            echo '<li class="sidebar-item contractor-sites-'.$activeUser['contractor_id'].'">'. $this->Html->link(__('The Hunton Group Specific<b>('.$percentage.'%)</b>'), ['controller'=>'ContractorSites', 'action'=>'addContact',$activeUser['contractor_id'],$activeUser['contractor_id']], ['escape' => false, 'class' =>'sidebar-link']). '</li>';
        }
    }
