...
class CategoryHelper extends Helper
{
    ...
    public function getServiceSubcat($service_id=null) {
	$this->Categories = \Cake\ORM\TableRegistry::getTableLocator()->get('Categories');

	$query = $this->Categories
	->find('all', fields: ['id','name'])
	->where(['service_id'=>$service_id,'year_based'=>true])
    ...
    public function getParentcat($contractor_id=null, $service_id=null, $parent_id=null)
    {
	$this->Categories = \Cake\ORM\TableRegistry::getTableLocator()->get('Categories');
	$this->ClientQuestions = \Cake\ORM\TableRegistry::getTableLocator()->get('ClientQuestions');
    ...
    public function getCategories($contractor_id=null, $service_id=null, $all=false, $category_id=null, $year=null)
    {			
	...
	$parentcatall = $this->getParentcat($contractor_id, $service_id, null);
	$allCat = array();

	$ifYearBased = $this->Categories->find('all', ['fields'=>['id','name']])->where(['id'=>$category_id,'year_based'=>true])->count();
	foreach($parentcatall as $parentcat) {
	// Yearbase categories
	if($all || $ifYearBased>0 || $year!=null) {
		...
		foreach($year_range as $year) {			
			...
			$childrens = $this->getParentcat($contractor_id, $service_id, $parentcat['id']);
			if(!empty($childrens)) {
	...
	// top categories and child categories
	if($all || ($ifYearBased==0 && $year==null)) {
		if($parentcat['year_based'] == false) {
			$allCat[$parentcat['id']] = $parentcat;
	...
	$this->Explanations = \Cake\ORM\TableRegistry::getTableLocator()->get('Explanations');
