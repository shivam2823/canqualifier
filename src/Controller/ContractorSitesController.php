...
class ContractorSitesController extends AppController
{
	...
	if (isset($user['role_id'])) {
		if(in_array($user['role_id'], array(SUPER_ADMIN, ADMIN, CONTRACTOR, CONTRACTOR_ADMIN, CR, CLIENT, CLIENT_ADMIN, CLIENT_BASIC))) {
			return true;
		}
	}
	// Default deny
    ...
    {
        $contractorSite = $this->ContractorSites->get($id, contain: ['Contractors', 'Clients', 'Sites']);
 ...
 //    foreach ($clients as $cl) {
	...
	//     }
 //    }
    $loggedIn = $this->getRequest()->getSession()->read('Auth.User');
    $whereClient = [];
    if(isset($loggedIn['role_id']) && in_array($loggedIn['role_id'], array(CLIENT, CLIENT_ADMIN)) && isset($loggedIn['client_id'])){
        $whereClient['Clients.id'] = $loggedIn['client_id'];
    }
	$userList = $this->Users
        ->find('all')
        ->contain(['ClientUsers'])
        ->contain(['ClientUsers.Clients' => ['conditions' => $whereClient]])
    ...
    foreach ($clients as $cl) {
        ...
        $whereClient = [];
        if(isset($loggedIn['role_id']) && in_array($loggedIn['role_id'], array(CLIENT, CLIENT_ADMIN)) && isset($loggedIn['client_id'])){
            $whereClient['Sites.client_id'] = $loggedIn['client_id'];
        }
	$contractorSites = $this->Contractors->get($contractor_id, 
		...
		'ContractorSites.Sites'=>['fields'=>['name'], 'conditions' => $whereClient],
	...
	$contractor = $this->Contractors->get($contractor_id, 
        ['contain'=>[
		'Users'=>['fields'=>['Users.id', 'Users.username']],
		'ContractorSites'=>['fields'=>['ContractorSites.site_id', 'ContractorSites.id','ContractorSites.contractor_id'],'conditions'=>['is_archived'=>false]],
		'ContractorSites.Sites'=>['conditions' => $whereClient, 'fields'=>['name','she_fname','she_lname','she_title','she_phone','she_email','facility_fname', 'facility_lname', 'facility_title', 'facility_phone', 'facility_email']],
    ...
    public function addContact($contractor_id = '', $activeClass = ''){

        $this->loadModel('ContractorSites');

        $this->loadModel('Sites');
        $this->loadModel('Contractors');
        $this->loadModel('ContractorUsers');
        $this->loadModel('Clients');
        $this->loadModel('SiteContacts');
            //Before save
        $contractorNav = true;
        $service_id = 1;
        $clientSites = array();
        $contractorSites = array();
        $existingSiteContacts = array();
            ...
            if(!empty($contractor_id)){
                ...
                if(!empty($contractorSites)){
                    ...
                    if(!empty($ConHuntonSites)){
                        $existingSiteContacts = $this->SiteContacts->find('all')->contain(['Sites' => ['fields' => ['name']]])->where(['site_id in' => array_keys($ConHuntonSites), 'contractor_id' => $contractor_id])->toArray();
                    }
                }
                //debug($existingSiteContacts);
            }
        ...
        if ($this->request->is(['patch', 'post', 'put'])) {
            ...
            foreach($newData as $record) {
                ...
                $existing = $this->SiteContacts->find()->select(['id'])->where(['site_id' => $record['site_id'], 'contractor_id' => $contractor_id])->first();

                if(!empty($existing->id)){
                    //edit
                    $siteContact = $this->SiteContacts->get($existing->id);
                    $siteContact = $this->SiteContacts->patchEntity($siteContact, $record);

                }else{
                    //add
                    $siteContact = '';
                    $siteContact = $this->SiteContacts->newEntity();
                    $siteContact = $this->SiteContacts->patchEntity($siteContact, $record);
                    $siteContact->contractor_id = $contractor_id;
                }
                if($result = $this->SiteContacts->save($siteContact)){
                    //$msg += 'Location contact for  added successfully.';
                }
            }
            $this->redirect(['controller'=>'ContractorAnswers', 'action'=>'addAnswers', 1, 2]);
        }
        $is_locked = 1;
        ...
        elseif($this->User->isContractor()) {
            $is_locked = $this->Contractors->find()->where(['id'=>$contractor_id, 'is_locked'=>true])->count();
        }
            $this->set(compact('ConHuntonSites', 'contractor_id', 'existingSiteContacts', 'contractorNav', 'service_id', 'is_locked'));
    }
