...
if(!empty($categories)) { ?>
    ...
    if($service_id == 1){
        ...
        if($service_id == 1 && in_array(6, $myClients)){
            echo '<li class="category-link contractor-sites-'.$activeUser['contractor_id'].'">'. $this->Html->link(__('The Hunton Group Specific('.$percentage.'%)'), ['controller'=>'ContractorSites', 'action'=>'addContact',$activeUser['contractor_id'],$activeUser['contractor_id']]). '</li>';
        }
    }
