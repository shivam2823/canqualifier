<?php if(isset($contractorNav) && $contractorNav == true && isset($activeUser['contractor_id'])) { ?>
	...
	<nav class="navbar navbar-expand-lg navbar-light bg-white">
	  <ul class="navbar-nav cat-nav">
		...
		<?php
		}elseif(!empty($service_id) && $service_id == 1 && in_array(6, $client)){
		    echo '<li class="nav-item">' .$this->Html->link(__($service_name), ['controller'=>'ContractorSites', 'action'=>'addContact',$activeUser['contractor_id'],$activeUser['contractor_id']], ['class'=>'nav-link btn btn-secondary']). '</li>';
        }
		else {
		?>
