jQuery(document).ready(function() {
    ...
    if (jQuery(".clientAddchk").length) {
        ...
        jQuery(".clientAddchk").change(function(){
            jQuery(this).parents('form').submit();
        });
    }
    ...
    jQuery(document).on("click", ".siteDelete", function(e) {
        ...
        jQuery.ajax({
            ...
            success: function(clientSiteCnt) {
                if (clientSiteCnt == 1) {
                    var x = alert("Client will be unassociated as this is the only site associated! Please add another site first to delete this site? Kindly note, contractor should be associated with atleast one client site/location.");
                    if (x) {
                        //selector.parents('form').submit();
                    }
                } else {
                    var x = confirm("Are you sure you want to remove the site?");
                    if (x) {
                        selector.parents('form').submit();
                    }
                }
